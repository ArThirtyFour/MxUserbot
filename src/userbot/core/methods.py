...
class Methods:
    ...
    async def set_room_avatar(
        ...
        if isinstance(result, RoomPutStateError):
            ...
            await self.send_text(self, room, f"sorry. can't set room avatar. I need at least be a moderator")
    ...
    async def send_msg(
        ...
        await self.send_text(self, msg_room, message)
        return True
    ...
    def on_invite_whitelist(bot, sender):
        invite_whitelist = bot.db.get("core", "invite_whitelist", [])
