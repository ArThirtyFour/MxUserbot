...
- `get_args_raw()` и `get_args()` умеют подтягивать текст из reply.
...
### `await decrypt_event(mx, event_to_decrypt, context_event=None) -> str | None`

- Назначение: пытается расшифровать `EncryptedEvent`.
- Аргументы:
  - `mx` — `MXBotInterface`;
  - `event_to_decrypt` — зашифрованный Matrix event;
  - `context_event` — исходное сообщение команды для ответа пользователю при ошибке.
- Возвращает:
  - `str`, если расшифровка успешна;
  - `None`, если ключа нет или произошла ошибка.
- Типичный сценарий: reply-команды, работающие в E2EE-комнатах.
- Caveats:
  - если ключа нет, helper может сам запросить его у устройств в фоне;
  - при переданном `context_event` сам отправляет пользователю статус через `utils.answer()`;
  - при ошибке не кидает исключение наружу, а возвращает `None`.
